...
class ToeApiControllerTest extends AuditApiTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->perencanaan = PerencanaanAudit::create([
            ...
            'nomor_surat_tugas' => 'ST/TOE-TEST/2026',
        ...
        $this->pka = ProgramKerjaAudit::create([
            ...
            'no_pka' => 'PKA/TOE-TEST/2026',
            'judul_pka' => 'PKA TOE Keuangan',
    ...
    /**
     * Test TOE resource CRUD, approval, and evaluation.
     */
    public function test_toe_crud_approval_and_evaluation(): void
    {
        $headers = $this->auditorHeaders();

        $payload = [
            'perencanaan_audit_id' => $this->perencanaan->id,
            'judul_bpm' => 'Prosedur Penerimaan Kas Utama (TOE)',
            'pemilihan_sampel_audit' => 'Sampel acak 10 transaksi',
            'hasil_evaluasi' => 'Efektif',
        ];

        // 1. Create (Store)
        $responseStore = $this->postJson('/api/v1/audit/toe', $payload, $headers);
        if ($responseStore->status() !== 201) {
            dump('TOE Store Error:', $responseStore->json());
        }
        ...
        // 2. Read (Show)
        $this->getJson("/api/v1/audit/toe/{$createdId}", $headers)
            ->assertStatus(200)
            ->assertJsonPath('data.judul_bpm', 'Prosedur Penerimaan Kas Utama (TOE)');

        // 3. Update
        $payloadUpdate = $payload;
        $payloadUpdate['judul_bpm'] = 'Prosedur Penerimaan Kas Utama (TOE) - Updated';

        $this->putJson("/api/v1/audit/toe/{$createdId}", $payloadUpdate, $headers)
            ->assertStatus(200)
            ->assertJsonPath('data.judul_bpm', 'Prosedur Penerimaan Kas Utama (TOE) - Updated');

        // 4. Index List
        $this->getJson('/api/v1/audit/toe', $headers)
            ->assertStatus(200)
            ->assertJsonPath('success', true);

        // 5. Approve (Level 1)
        $this->postJson("/api/v1/audit/toe/{$createdId}/approval", [
            'action' => 'approve'
        ], $this->asKetuaTim($this->auditor))
            ->assertStatus(200)
            ->assertJsonPath('success', true);

        $this->assertEquals('approved_level1', ToeAudit::find($createdId)->status_approval);

        // 6. Evaluasi Store
        $responseEval = $this->postJson('/api/v1/audit/toe-evaluasi', [
            'toe_audit_id' => $createdId,
            'hasil_evaluasi' => 'Evaluasi awal menunjukkan kontrol efektif.'
        ], $headers);
        ...
        // 7. Evaluasi Index
        $this->getJson("/api/v1/audit/toe/{$createdId}/evaluasi", $headers)
            ->assertStatus(200)
            ->assertJsonPath('success', true);

        // 8. Evaluasi Update
        $this->putJson("/api/v1/audit/toe-evaluasi/{$evalId}", [
        ...
        // 9. Evaluasi Destroy
        $this->deleteJson("/api/v1/audit/toe-evaluasi/{$evalId}", [], $headers)
            ->assertStatus(200)
            ->assertJsonPath('success', true);

        // 10. Delete (Destroy)
        $this->deleteJson("/api/v1/audit/toe/{$createdId}", [], $headers)
            ->assertStatus(200)
            ->assertJsonPath('success', true);
