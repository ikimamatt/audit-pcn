        ...
        <div class="card">
            <div class="card-body">
                <form method="GET" class="mb-3 d-flex align-items-center" action="">
                    ...
                    <a href="{{ route('audit.tod-bpm.index') }}" class="btn btn-secondary ms-2">
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($item->perencanaanAudit)
                                        {{ \Carbon\Carbon::parse($item->perencanaanAudit->tanggal_audit_mulai)->format('d/m/Y') }} s/d 
                                ...
                                <td>
                                    ...
                                    <div class="d-flex gap-1">
                                        <a href="{{ route('audit.tod-bpm.edit', $item->id) }}" class="btn btn-warning btn-sm text-white shadow-sm" title="Edit">
                                            <i class="mdi mdi-pencil"></i>
                                        </a>
                                        <form action="{{ route('audit.tod-bpm.destroy', $item->id) }}" method="POST" class="delete-form m-0">
                                            @csrf @method('DELETE')
                                        ...
                                        @if($item->status_approval == 'pending')
                                            ...
                                            @isKspi
                                                ...
                                                <form id="approval-form-{{ $item->id }}" action="{{ route('audit.tod-bpm.approval', $item->id) }}" method="POST" style="display:inline-block">
                                                    @csrf
                                                    @if($hasAsmanKspi)
                                                        <button type="button" class="btn btn-success btn-sm" onclick="approveDataPending({{ $item->id }})" title="Data harus diapprove oleh ASMAN KSPI terlebih dahulu">
                                                            <i class="mdi mdi-check"></i> Approve Level 2
                                                        </button>
                                                    @else
                                                        <button type="button" class="btn btn-success btn-sm" onclick="approveData({{ $item->id }})" title="Approve langsung (tidak ada ASMAN KSPI)">
