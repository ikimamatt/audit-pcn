                                    ...
                                    @if($item->status_approval == 'pending')
                                        ...
                                        @isAsmanKspi
                                            <form id="approval-form-{{ $item->id }}" action="{{ route('audit.tod-bpm.approval', $item->id) }}" method="POST" class="m-0" style="display:inline-flex; gap:5px;">
                                                ...
                                                <button type="button" class="btn-act btn-act-approve" onclick="approveData({{ $item->id }})" title="Approve Level 1">
                                                    <i class="mdi mdi-check"></i>
                                                </button>
                                                <button type="button" class="btn-act btn-act-reject" onclick="rejectData({{ $item->id }})" title="Reject Level 1">
                                        ...
                                        @isKspi
                                            ...
                                            <form id="approval-form-{{ $item->id }}" action="{{ route('audit.tod-bpm.approval', $item->id) }}" method="POST" class="m-0" style="display:inline-flex; gap:5px;">
                                                ...
                                                <input type="hidden" name="action" id="action-{{ $item->id }}" value="">
                                                @if($hasAsmanKspi)
                                                    <button type="button" class="btn-act btn-act-approve" onclick="approveDataPending({{ $item->id }})" title="Data harus diapprove oleh ASMAN KSPI terlebih dahulu">
                                                        <i class="mdi mdi-check"></i>
                                                    </button>
                                                @else
                                                    <button type="button" class="btn-act btn-act-approve" onclick="approveData({{ $item->id }})" title="Approve langsung (tidak ada ASMAN KSPI)">
                                                        <i class="mdi mdi-check"></i>
                                                    </button>
                                                @endif
                                                <button type="button" class="btn-act btn-act-reject" onclick="rejectData({{ $item->id }})" title="Reject Level 2">
                                                    <i class="mdi mdi-close"></i>
                                                </button>
                                            </form>
                                        @endisKspi
                                    @elseif($item->status_approval == 'approved_level1')
                                        ...
                                        @isKspi
                                            <form id="approval-form-{{ $item->id }}" action="{{ route('audit.tod-bpm.approval', $item->id) }}" method="POST" class="m-0" style="display:inline-flex; gap:5px;">
                                                @csrf
                                                <input type="hidden" name="action" id="action-{{ $item->id }}" value="">
                                                <button type="button" class="btn-act btn-act-approve" onclick="approveData({{ $item->id }})" title="Approve Level 2">
                                                    <i class="mdi mdi-check"></i>
                                                </button>
                                                <button type="button" class="btn-act btn-act-reject" onclick="rejectData({{ $item->id }})" title="Reject Level 2">
                                                    <i class="mdi mdi-close"></i>
