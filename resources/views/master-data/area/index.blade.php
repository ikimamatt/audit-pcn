@extends('layouts.vertical', ['title' => 'Master Area'])
    ...
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
        <div>
            <div class="d-flex align-items-center gap-2 mb-1">
                <i class="mdi mdi-map-marker-radius-outline" style="font-size:1.4rem; color:#2d6a9f;"></i>
                <h2 class="mb-0">Master Area</h2>
            </div>
            <div class="subtitle">
                <i class="mdi mdi-home-outline me-1"></i>Home &rsaquo; Master Data &rsaquo; Area
            </div>
        </div>
        <a href="{{ route('master.area.create') }}" class="btn-add-em">
            <i class="mdi mdi-plus-circle"></i> Tambah Area
        </a>
    </div>
</div>
        ...
        <div class="table-responsive">
                ...
                <tbody>
                    @forelse($data as $index => $item)
                        <tr>
                            ...
                            <td>
                                @if($item->region)
                                    <span class="badge bg-info-subtle text-info px-2 py-1" style="font-size:0.75rem; border-radius:6px;">
                                        [{{ $item->kd_region }}] {{ $item->region->nama_region }}
                                    </span>
                                @else
                                    <span class="badge bg-light text-dark border-0 px-2 py-1" style="font-size:0.75rem; border-radius:6px;">-</span>
                                @endif
                            </td>
                            <td>
                            ...
                            <td class="fw-semibold text-dark">{{ $item->nama_area }}</td>
                            <td>
                                <div class="action-wrap">
                                    <a href="{{ route('master.area.edit', $item->id) }}"
                                       class="btn-act btn-act-edit"
                                       title="Edit Area">
                                        <i class="mdi mdi-pencil"></i>
                                    </a>
                                    <form id="delete-form-{{ $item->id }}" action="{{ route('master.area.destroy', $item->id) }}" method="POST" class="m-0">
                                        @csrf @method('DELETE')
                                        <button type="button"
            ...
            @if($data->isEmpty())
                <div class="empty-state">
                    ...
                    <p class="mb-0 fw-semibold">Belum ada data Area</p>
                    <p class="mb-0" style="font-size:.82rem;">Klik tombol <strong>Tambah Area</strong> untuk memulai</p>
                </div>
            @endif
        </div>
    </div>
