    ...
    <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
        <div>
            <div class="d-flex align-items-center gap-2 mb-1">
                <i class="mdi mdi-format-list-bulleted-type" style="font-size:1.4rem; color:#2d6a9f;"></i>
                <h2 class="mb-0">Master Jenis Audit</h2>
            </div>
            <div class="subtitle">
                <i class="mdi mdi-home-outline me-1"></i>Home &rsaquo; Master Data &rsaquo; Jenis Audit
            </div>
        </div>
        <a href="{{ route('master.jenis-audit.create') }}" class="btn-add-em">
            <i class="mdi mdi-plus-circle"></i> Tambah Jenis Audit
        </a>
    </div>
</div>
...
<div class="card table-card">
    <div class="card-header-custom d-flex align-items-center justify-content-between pb-3">
        <div class="d-flex align-items-center gap-2">
            ...
            <h5 class="mb-0">Daftar Jenis Audit</h5>
        </div>
        ...
        <div class="table-responsive">
                ...
                <tbody>
                    @forelse($data as $index => $item)
                        <tr>
                            ...
                            <td class="fw-semibold text-dark">{{ $item->nama_jenis_audit }}</td>
                            <td>
                            ...
                            <td>
                                <div class="action-wrap">
                                    <a href="{{ route('master.jenis-audit.edit', $item->id) }}" 
                                       ...
                                       title="Edit Jenis Audit">
                                        <i class="mdi mdi-pencil"></i>
                                    </a>
                                    <form id="delete-form-{{ $item->id }}" action="{{ route('master.jenis-audit.destroy', $item->id) }}" method="POST" class="m-0">
                                        @csrf @method('DELETE')
                                        <button type="button"
            ...
            @if($data->isEmpty())
                <div class="empty-state">
                    ...
                    <p class="mb-0 fw-semibold">Belum ada data Jenis Audit</p>
                    <p class="mb-0" style="font-size:.82rem;">Klik tombol <strong>Tambah Jenis Audit</strong> untuk memulai</p>
                </div>
            @endif
        </div>
    </div>
